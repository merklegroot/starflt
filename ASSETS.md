...
| **Saturn ring** (`2k_saturn_ring_alpha.png`) | `StarGame/Textures/solarsystemscope/` · attribution: `ATTRIBUTION.txt` | [Solar System Scope — Solar Textures](https://www.solarsystemscope.com/textures/) | [**CC BY 4.0**](https://creativecommons.org/licenses/by/4.0/) — “You may use, adapt, and share these textures for any purpose, even commercially” (see site). | High-res download URL used in-repo: `https://www.solarsystemscope.com/textures/download/2k_saturn_ring_alpha.png`. Used for **all** ringed planets for now (see `RingTextureResources`); `planets.json` `particleTexture` is ignored until more textures exist. **MIT + CC BY:** game *source* stays MIT; this *asset* stays under CC BY 4.0—keep `ATTRIBUTION.txt` (or equivalent credit) with distributions that include the PNG. |
