...
| **Jupiter** (`jupiter.png`, `jupiter.jpg`) | `StarGame/Textures/solarsystemscope/` · `ATTRIBUTION.txt` | [Solar System Scope — Solar Textures](https://www.solarsystemscope.com/textures/) | [**CC BY 4.0**](https://creativecommons.org/licenses/by/4.0/) | **Source:** `https://www.solarsystemscope.com/textures/download/2k_jupiter.jpg` (2K equirectangular). **In-repo:** **1024×512** PNG (primary runtime asset, converted from the JPEG) plus 1024×512 JPEG fallback. Wired when `Planet.Name` is Jupiter (see `PlanetSphereTextureResources` + equirectangular UVs in `Planet`). |
